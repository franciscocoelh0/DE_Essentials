...
def read_csv(file, schemas):
    file_path_list = re.split('[/\\\]', file) # Error handling for both Windows and Unix file paths
...
def file_converter(src_base_dir, tgt_base_dir, ds_name):
    ...
    if len(files) == 0:
        raise NameError(f'No files found for dataset {ds_name} in {src_base_dir}/{ds_name}')

    for file in files:
        df = read_csv(file, schemas)
        file_name = re.split('[/\\\]', file)[-1] # Error handling for both Windows and Unix file paths
